...
|类型名(英文)|类型名(中文)|简要解释|
|-|-|-|
|`pair<type1, type2>`|结对型|将2个数据组合成一个数据(`pair` 后面可以有空格)|
...
比较复杂的常用类型的声明、赋值以及使用如下：

```c++
pair<int, string> p1 = make_pair(1, "Hello World!");
pair<char, string> p2[10];
p1[1] = make_pair('a', "ab");
print(p1[1].first); // 输出 a
print(p.last);      // 输出 Hello World
```
