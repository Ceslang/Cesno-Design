...
**类型运算符**可以将两个类型组合成新的类型——这些类型多半不用在声明上，而是约束参数的类型，或是表明函数返回值。
...
这个运算符可以判断左侧的值是否**非空值**，返回`true`或`false`。一般的，`null`、`undefined`被认为是**空值**。

`(value_to_check ??)`相当于`(value_to_check != null and value_to_check != undefined)`。这很像JavaScript中的`??`的检测范围。

### 非空非零检测运算符 `???`

这个运算符相当于`??`运算符多一步**零值检测**，即检测左侧值是否是`null`、`undefined`，或是属于该对象所属类型中定义的零值`$zero_values`。

`(value_to_check ???)`相当于`(value_to_check != null and value_to_check != undefined and value_to_check not in value_to_check.$zero_values)`。这很像C或者JavaScript对if语句的隐式类型转换。

### 真值外补偿提示符`:` (限定在`??`或`???`后)

使用形如`x ??: y`或`x ???: y`的形式，可以在检测`x`后，根据条件提供不同值。当`??`或`???`运算结果为`false`时，该式评价为`y`，否则为`x`。

### 值舍弃运算符`void`

这个运算符执行右侧的表达式，并舍弃掉结果，返回`void`。
...
运算符可能会被“借用”，产生一些貌似不符合定义上的用法。这种现象叫做运算符的重载。比如，列表的